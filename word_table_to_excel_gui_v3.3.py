...
def con_fn():
    if len(sel.get())>0 and len(sav.get())>0 and len(top_ent.get())>0 and len(tra_ent.get())>0:
        ...
        if  dira:
            ...
            for k in range(len(onlyfiles)):
                ...
                file_list.append((onlyfiles[k],spath,dira,title,task))
                
            
        else:
            file_list.append((path,spath,dira,title,task))
    ...
    while True:
        ...
        try:
            if file_list:
                ...
                lent=0
                heads=["Type","Title","Track","Topic","Complexity","content"]
                sel=file_list.pop()
                ...
                try:
                    ...
                    for k in data_cor:
                        ...
                        try:
                            ...
                            new_data.append(sel[4])
                            new_data.append(sel[3])
                            if titil:
                                q=titil.pop();
                                flag=0
                                comp=""
                                topic=""
                                for s in ["Easy","Hard","Medium","easy","hard","medium"]:
                                    if s in q:
                                        comp=s
                                        topic=q.replace(s,"")
                                if  topic == "":
                            ...
                            for p in range(len(ind)):
                                ...
                                #print(str(k[ind[p]])+str(k[ind[p]+2])+"--",ind)
                                
                                if float(str(k[ind[p]+2])) > 0:
                                    #print("hi")
                                    if ans == "0":
                                        ans = str(p+1)
                                    else:
...
win = tk.Tk() 
win.title("word_table_to_excel V 3.3") 
...
top_ent = tk.Entry(win,font = ("Times New Roman", 13) ,width=30)
tra_ent = tk.Entry(win,font = ("Times New Roman", 13) ,width=30) 
...
con_start = tk.Button(win, text='Start Conversion ', font = ("Times New Roman", 15),background = '#5BF54C',width=15,  padx=2 ,pady=2,command=con_fn).grid(row=6, pady = 15, column=1)
sel.grid(row=0, column=1) 
sav.grid(row=3, column=1)
top_ent.grid(row=4,column=1,pady=15)
tra_ent.grid(row=5,column=1,pady=15)
...
tk.Label(win,  
         ...
         font = ("Times New Roman", 15),  
         background = '#EDF568',  
         foreground = "black").grid(column = 3,row=7,columnspan=2 )
...
staus_area.grid(column = 3,row=8,columnspan=2, pady = 10, padx = 10) 
